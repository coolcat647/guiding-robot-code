...
class hapticSnedNode(object):
    def __init__(self):
        ...
        self.mac = rospy.get_param('~mac', "20:15:04:30:61:95")    # 98:D3:31:F4:0C:89
    ...
    def send(self):
        ...
        frequency = BitArray(uint=self.frequencies[2] if (self.frequencies[2]<4 and self.frequencies[2]!=0) else 4, length=3)
        intensity = BitArray(uint=self.intensities[2] if self.intensities[2]< 5 else 5, length=3)
        state     = BitArray(uint=1, length=3)
        toSend.append(frequency)
        toSend.append(intensity)
        toSend.append(state)

        frequency = BitArray(uint=3, length=3)
        ...
        frequency = BitArray(uint=self.frequencies[0] if (self.frequencies[0]<4 and self.frequencies[0]!=0) else 4, length=3)
        intensity = BitArray(uint=self.intensities[0] if self.intensities[0]< 5 else 5, length=3)
        state     = BitArray(uint=1, length=3)
        toSend.append(frequency)
        toSend.append(intensity)
        toSend.append(state)
        ...
        frequency = BitArray(uint=self.frequencies[1] if (self.frequencies[1]<4 and self.frequencies[1]!=0) else 4, length=3)
        intensity = BitArray(uint=self.intensities[1] if self.intensities[1]< 5 else 5, length=3)
        state     = BitArray(uint=1, length=3)
        toSend.append(frequency)
        toSend.append(intensity)
        toSend.append(state)
        ...
        toSend.append('0b000')
        toSend.append('0x00')
        # toSend.append(BitArray(uint=0, length=11))
        ...
        if self.enable_bluetooth:
            ...
            # self.bt.send(self.toConnect, toSend)
